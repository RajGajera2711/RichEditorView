...
@objcMembers open class RichEditorView: UIView, UIScrollViewDelegate, UIWebViewDelegate, UIGestureRecognizerDelegate {
    ...
    open var FontFamily: String {
        ...
        set {
            ...
            runJS("RE.changeFontFamily('\(newValue.escaped)');")
          //  runJS("RE.fontchange('\(newValue.escaped)');")
        }
    }
    ...
    public func fontchange() {
        
        let arrFont = ["Amatic SC","Dancing Script","Great Vibes","Opificio","Avenir"]
        let randomIndex = Int(arc4random_uniform(UInt32(arrFont.count)))
        
        FontFamilyName = arrFont[randomIndex]
        runJS("RE.changeFontFamily('\(FontFamilyName)');")
